...
function prevMusic() {
 ...
 musicIndex--;
 // if musicIndex is less than 1 then musicIndex will be arrary length  so the last song will play
...
// above we just changed to the icon, now let's work on what to do after the song ended
...
mainAudio.addEventListener("ended", () => {
 ...
 switch (getText) {
  case "repeat": // if this icon is repeat then we call the nextMusic function so the next song will play
   nextMusic();
   break;
  case "repeat_one": // if icon is repeat_one then we'll change the current playing song current time to 0 so song will play from begining
  ...
  case "shuffle": // if icon is shuffle then change it to repeat
   ...
   } while (musicIndex == randIndex); // this loop run until the next random number won't be the same as the current music index
...
for (let i = 0; i < allMusic.length; i++) {
 ...
 let liTag = `<li li-index = "${i + 1}">
               ...
               <audio class ="${allMusic[i].src}" src="sounds/${
  allMusic[i].src
 }.mp3"></audio>
               <span id ="${
                allMusic[i].src
